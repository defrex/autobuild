...
export const HELP_CATALOG: readonly HelpEntry[] = [
  ...
  },
  {
    ...
    detail: page([
      ...
      'fresh disposable remote sandbox for registered remote providers; the environment is',
  ...
  },
  {
    ...
    detail: page([
      ...
      '',
      'By default, update the running Bun installation to its latest published',
      'release — the npm registry for a registry install, the repository’s GitHub',
      'Release for a github: install — then use the replacement binary and defaults to',
  ...
  },
  {
    name: 'watch',
    audience: 'human',
    summary: 'Stream build and repository events as they land, with a resumable cursor.',
    detail: page([
      'Usage:',
      '  ab watch [<slug>...] [--repository] [--event <glob>]... [--since <cursor>] [--timeout <duration>] [--interval <duration>] [--count <n>] [--json] [--store <ref>]',
      '',
      'Follow the event logs of one, several, or all active builds in this repository —',
      'optionally the repository journal — and emit one record per matching event as it',
      'lands. With no slug, every nonterminal build (queued, running, paused, blocked) is',
      'watched, and a build created while the watch runs joins it without a restart.',
      'Naming one or more slugs watches exactly those builds; an unknown slug is an error',
      'before any record is emitted, and the watch ends when every named build reaches a',
      'terminal status. With no slugs, an empty set of active builds does not end the watch.',
      '',
      'Options:',
      '  --repository       Also follow the repository journal (dispatcher and harvest',
      '                     events). Without it, repository events are never emitted.',
      '  --event <glob>     Repeatable glob over event type names: * matches any run of',
      '                     characters, ? one character (escalation.*, pr.merged, *). Every',
      '                     glob must match at least one type in the build catalog — or,',
      '                     with --repository, the repository catalog — or the command fails',
      '                     before any store read. Any --event replaces the default filter.',
      '  --since <cursor>   Resume from a cursor emitted by an earlier record; every matching',
      '                     event after that cursor is delivered exactly once, including',
      '                     events that landed while no watch was running. A cursor from a',
      '                     different repository or store is rejected.',
      '  --timeout <dur>    End the watch after this duration (30, 45s, 5m, 1h). Default',
      '                     30 minutes; 0 means unbounded.',
      '  --interval <dur>   Poll cadence. Default: 5s for an http(s) store, 1s for a local store.',
      '  --count <n>        End the watch after n records.',
      '  --json             Newline-delimited JSON: exactly one object per matching event,',
      '                     flushed as soon as it is written.',
      '  --store <ref>      Override the BuildStore path or URL.',
      '',
      'Default filter — the attention set. For builds: escalation.raised, phase.failed,',
      'infrastructure.failed, runner.setup-failed, dispatch.failed, publication.lost,',
      'finalize.completed, pr.conflicted, pr.merged, pr.closed, build.completed,',
      'build.aborted. For the repository journal: harvest.escalated, harvest.failed,',
      'dispatcher.tick-failed, dispatcher.config-rejected, dispatcher.harvest-runner-failed.',
      '',
      'Record shape (--json): build (slug, or null for a repository event), repo (repository',
      'id, present only for repository events), event (the complete durable envelope: build,',
      'seq, ts, actor, type, payload), state (for build events, the reduction after this',
      'event: status, phase, round, openEscalations with their ids and questions, pr, prState,',
      'outcome; null for repository events), and cursor. Without --json, each record is one',
      'human-readable line with no ANSI sequences. On every exit, the last --json line on',
      'stdout is a bare {"cursor": …} object reflecting the final position, so a caller can',
      'resume even if no record matched. Cursors are opaque strings; their format is not part',
      'of the contract.',
      '',
      'Exit codes: the watch ends with exit 0 when --timeout elapses, when --count records',
      'have been emitted, or, only when slugs were named, when every named build has reached a',
      'terminal status. Usage errors, scope violations, unknown slugs, cursor rejection, and a',
      'store that cannot be opened at start exit 1 with a message on stderr. A store read',
      'that fails after the watch has started is reported on stderr',
      'once per failing read source — locally the poll cycle is one source, while against',
      'an http(s) store each tracked stream and the discovery pass carries its own failure',
      'streak — and retried at the next interval without ending the watch or duplicating or',
      'skipping any event.',
      '',
      'This command is read-only: it appends no event, takes no lease, and creates no build or',
      'repository record, and requires no session identity for operator use. Inside a phase, a',
      'complete ambient tuple permits only the ambient build: the no-slug form, a foreign slug,',
      'and --repository are rejected, as are malformed or partial identity.',
    ]),
  },
  {
    name: 'wait',
    audience: 'human',
    summary: 'Block until a build satisfies a condition, then print one record.',
    detail: page([
      'Usage:',
      '  ab wait [<slug>...] (--for <condition> | --event <glob>)... [--since <cursor>] [--timeout <duration>] [--interval <duration>] [--json] [--store <ref>]',
      '',
      'Block until a targeted build satisfies a condition, print exactly one record describing',
      'the event and the build’s reduced state, and exit with a code that tells the caller',
      'what happened. It is the blocking sibling of `ab watch` and shares its target',
      'selection, cursor, record shape, and ambient scoping. At least one --for or --event is',
      'required. With no slug, every nonterminal build is targeted — including builds created',
      'while waiting — and an empty or entirely terminal set keeps waiting for a build to',
      'appear. Naming one or more slugs targets exactly those builds; an unknown slug is an',
      'error before waiting begins.',
      '',
      'Conditions (--for, repeatable): blocked, paused, running, done, aborted, terminal (done',
      'or aborted), pr=open, pr=merged, pr=closed, pr=conflicted, phase=<phase> (a core phase —',
      'plan, plan-review, implement, code-review, finalize, reconcile — or verify:<step>), and',
      'attention (any event in the ab watch attention set). An unknown value is rejected before',
      'any store access. Repeatable --event globs follow the ab watch rules. Multiple --for',
      'values and --event globs are alternatives: the first one satisfied ends the wait — all',
      '--for values in supply order first (each checked against the state after an event and',
      'the event’s attention membership), then all --event globs in supply order.',
      '',
      'A status, PR-lifecycle, or phase condition is satisfied when a targeted build’s reduction',
      'has that status, PR state, or current phase. If any targeted build already satisfies',
      'such a condition when the command starts, the command returns immediately — in the',
      'manner of kubectl wait — with that build’s latest event; --since does not suppress this.',
      'Otherwise the condition is satisfied by the first later event whose reduction produces',
      'it. attention and --event conditions are satisfied only by events appended after the',
      'command started (or, on a cursor-resumed backlog, after --since).',
      '',
      'Record shape (--json): the ab watch record — build, event (the complete durable',
      'envelope), state (the reduction after this event: status, phase, round, openEscalations,',
      'pr, prState, outcome), cursor — plus condition, naming the --for value or --event glob',
      'that matched. Without --json, one human-readable line with the condition in parentheses',
      'and no ANSI sequences. Unlike ab watch, no trailing cursor object is emitted: the',
      'record’s own cursor is the final position, and `ab wait --since <cursor>` resumes.',
      '',
      'Exit codes: the wait ends with exit 0 when a condition is satisfied and exactly one',
      'record was printed; exit 1 on usage errors, scope violations, unknown slugs, or a store',
      'that cannot be opened at start; exit 2 when slugs were named and every named build',
      'reached a terminal status without satisfying any condition — one record whose condition',
      'is null, showing how the build ended; exit 3 when --timeout elapsed (default 30 minutes;',
      '0 unbounded) — nothing on stdout except, with --json, a bare {"cursor": …} object for',
      'the final position, and one line on stderr; exit 4 when interrupted by SIGINT before',
      'any condition was satisfied, with the same cursor behavior as exit 3.',
      '',
      'The default poll interval and the --interval override match ab watch: 5s for an http(s)',
      'store, 1s for a local store. Cadence follows the store kind, as in ab watch: a local',
      'store ticks sequentially at the interval, while against an http(s) store each targeted',
      'build keeps one held read in flight — one long-held request per wait window (AUT-334),',
      'so an appended event is seen within about a second — with --interval the floor between',
      'request starts. A held read’s window is the remote default (25s, whole seconds, clamped',
      'by the store’s ceiling), capped at the remaining --timeout budget, and every stop — a',
      'satisfied condition, an all-terminal named set, the timeout, an interrupt — cancels',
      'in-flight holds promptly. A store read that fails while waiting is reported on stderr',
      'once per failing read source — locally the poll cycle is one source, while against an',
      'http(s) store each tracked build and the discovery pass carries its own failure',
      'streak — retried at the next interval (the next request on the remote path), and never',
      'causes a condition to be missed or reported twice.',
      '',
      'This command is read-only: it appends no event, takes no lease, and creates no build or',
      'repository record, and requires no session identity for operator use. Inside a phase, a',
      'complete ambient tuple permits only the ambient build; foreign builds and malformed or',
      'partial identity are rejected before any store access.',
    ]),
  },
  {
    name: 'mcp',
    audience: 'human',
    summary: 'Serve the operator tool registry as an MCP server over stdio.',
    detail: page([
      'Usage:',
      '  ab mcp [--store <ref>] [--repo <id>] (§8.2)',
      '',
      'Serve the agent tool registry as an MCP server on stdio: every tool',
      'mirrors an operator CLI command or operator API route, with the same',
      'schemas, annotations, and results. The CLI remains the primary local',
      'interface for agents that have a shell; this binding exists so the',
      'registry can be exercised end to end from a local install, for',
      'dogfooding, and for local agents that cannot run commands.',
      '',
      'Options:',
      '  --store <ref>      Override the BuildStore path or URL.',
      '  --repo <id>        Constrain the server: calls naming any other',
      '                     repository are refused.',
      '',
      'Mutating tools attribute their writes to the sessionless operator',
      'identity (USER/USERNAME, else "dashboard"), exactly like the other',
      'operator commands. The server fails closed inside a phase: repository-',
      'wide operator authority cannot be narrowed to the ambient build, so a',
      'complete or malformed AB_* phase tuple refuses to start the server.',
      'The server exits when its client disconnects (stdio EOF).',
    ]),
  },
  {
    name: 'pause',
  ...
  },
  {
    ...
    detail: page([
      ...
      '  ab answer <slug> [<text>] --review-round-ceiling <n> [--store <ref>]',
      ...
      'a retry.',
      '',
      '--review-round-ceiling accepts a positive absolute ceiling only while one open',
      'review-round-limit escalation selects the plan or code loop. The same answer',
      'durably sets that build/loop/current-spec override and clears the escalation.',
      'It cannot be used pre-emptively or with either spec-revision form, because a',
      'revision resets the loop budget itself. The configured maxReviewRounds remains',
      'the default for every other build and loop; stallRounds is unaffected.',
      '',
  ...
  },
  {
    ...
    detail: page([
      ...
      '  ab artifact download stream:<id>[@rev] --output <file> [--store <ref>]',
      '',
      ...
      '',
      'The stream:<id> form retrieves a finalized repository-scoped stream artifact',
      "(for example a harvest session's closed stream). The stream id is globally",
      'unique, so no build argument is needed; find ids in the session rows of',
      "'ab harvest status'. This form is strictly an operator read: any ambient",
      'build or Harvest identity fails closed (§8.2).',
    ]),
  },
  {
    name: 'observe',
