...
export const HELP_CATALOG: readonly HelpEntry[] = [
  ...
  },
  {
    ...
    detail: page([
      ...
      'store, 1s for a local store. A store read that fails while waiting is reported once on',
      'stderr, retried at the next interval, and never causes a condition to be missed or',
      'reported twice.',
      '',
      'This command is read-only: it appends no event, takes no lease, and creates no build or',
      'repository record, and requires no session identity for operator use. Inside a phase, a',
