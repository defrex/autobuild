...
Before the first frame, neutral placeholders inside the operator control line reserve its loaded footprint, and five static neutral placeholders reserve the build-list register. Each row placeholder occupies the same four-row rhythm as a normal build row and uses only slack/well geometry: no digits, status words, imperative, synthetic values, or animation. Decorative geometry is hidden from assistive technology; one visually hidden polite message announces loading.
...
### Navigation
- **Style:** one operator control line, in this order: the `repo` selector only when two or more repositories are configured; queue, active, and observation counts; the optional `repository PAUSED` warning; intake, auto merge, and harvest toggle words; then identity and `sign out` in slack pinned right. Running repositories render no repository-state token. The `repo` label and selector are both absent when there is no choice.
- **Mobile:** the line wraps only between whole items; nothing clips or collapses into a menu. Identity and `sign out` stay together on their own full-width, right-aligned line below 720px.
...
- **Secondary / ghost (`word`):** transparent and borderless at rest, inheriting the surrounding ink. This covers the build title and auto-merge indicator, row controls, close detail, cancel, sign out, open transcript, and operator toggle words. Row controls use uppercase action vocabulary and sit in reserved in-grid space rather than an overlay. Hover turns cyan and underlines; focus-visible gets the cyan ring; active temporarily fills ink with ground text (including nested state text); disabled drops to slack and keeps a transparent ground with a not-allowed cursor.
...
### Build Row
Lane (`>` in cyan), ticket id and bold slug in a direct detail-toggle button, right-pinned tokens, and the bold STATUS word in its status color pinned to the right edge. The title has a slug-qualified accessible name, expanded state, and visible keyboard focus. The token cluster always includes a direct auto-merge ghost-word button whose visible OFF, REQUESTED, ENABLED, or CANCELLING word and color communicate state together; its pressed state represents the desired setting, and availability or an in-flight request disables it. It toggles only auto merge and never opens detail. Pipeline and message previews sit immediately below the headline. A fixed one-row, in-flow register beneath all previews reserves room only for the lifecycle or destructive actions available to that status: ABORT plus PAUSE, CANCEL PAUSE, RESUME, or DISCARD. Harvest reserves the same register for its available RESUME or ACKNOWLEDGE run action; the repository Harvest gate remains global. Register words are hidden with visibility at rest, and become visible for committed selection, keyboard focus within the row, or a fine-pointer hover. Hidden words leave the tab order; title and auto-merge controls remain directly focusable, and focusing within the row reveals the register. The register remains one row at narrow width because every substitution has at most two actions. Each control's accessible name includes its build slug or Harvest run, and activating it commits that target without toggling detail; only the title opens or closes detail. Committed selection draws the lane marker bold; on devices with a fine hovering primary pointer, hovering a build or Harvest row draws a regular-weight preview marker in the same reserved lane, and leaving the list removes it. Selection and preview may appear on different rows at once; only committed selection drives focus dimming, detail, and ARIA state. The step line carries each step as glyph, label, and a parenthesised note (`plan(3s)`, `merge(waiting, 8m47s)`) in the terminal's duration format `38s`, `4m12s`, `1h04m`. A build-owned message shows as a three-row preview: `!` on the first row, two-space indent on the rest, a `... N more rows - Enter details` line when truncated, all in the message's tone. Hover turns the identity cyan. When any row is selected, every other row dims to slack except its STATUS word, its yellow `(held)` annotation, and its red lines; the hold remains full-color because it is decision-relevant. An open blocked-build answer is exclusive to the selected row: its register exposes only SUBMIT and CANCEL, so RESUME, ABORT, auto merge, detail toggling, and their row shortcuts cannot interrupt a draft. Controls on every other row retain their normal target-aware behavior and leave the old answer context when activated.
...
- **Do** keep build registers beneath every pipeline and message preview, limited to two lifecycle/destructive words; use the title for detail and the always-visible state token for auto merge.
...
- **Don't** put hidden controls between a build title and its progress, or duplicate detail and auto-merge actions in the row register.
