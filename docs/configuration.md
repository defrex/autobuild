...
Set `[tickets].source` to a registered ticket adapter name. Agent runtime and
workspace selectors remain restricted to shipped builtins. Forge selection is
also open: set the root `forge` scalar to a registered name; omission selects
`github`. A selected plugin forge factory receives an empty adapter-specific
`config` object, the process environment, and the absolute repository root. It
is invoked lazily after the complete plugin catalog has loaded and before the
...
`prAttachments` capability intentionally selects text-only attachment
summaries, while a present capability serves upload and terminal reclamation.
