...
| Provisioning field | Constraints |
|---|---|
...
credential. After Bun, the Autobuild distribution, and checkout dependencies, provisioning
runs each referenced runtime's `install` then `preflight` command in sorted
runtime-name order from the checkout. Only `environmentVariables` values enter
those commands. The same sequence runs in fresh readiness sandboxes and every
replacement sandbox. Provisioning writes its readiness marker only after every
runtime preflight completes; distribution packing/install disables package lifecycle scripts so
