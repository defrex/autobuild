...
export const HELP_CATALOG: readonly HelpEntry[] = [
  ...
  },
  {
    ...
    detail: page([
      ...
      'dashboard’s global-row p: it durably turns ticket intake off, then requests a',
      'pause on every running build that does not already have one. Queued, pausing,',
      'paused, blocked, and finished builds are skipped without error, and a pending',
      'pause is never cancelled. Output names every build that received a request and',
      'the intake value written; --json emits that summary as JSON. Re-running it',
      'changes nothing further. A phase session may not use --all. A failure exits',
      'nonzero and reports which builds were requested and which were not.',
    ]),
  },
  {
    name: 'resume',
    ...
    detail: page([
      ...
      '',
      '--all is the repository-wide form and the sessionless equivalent of the',
      'dashboard’s global-row r: it durably turns ticket intake back on, then requests',
      'a resume on every cleanly paused build that does not already have one. Running,',
      'resuming, queued, blocked, and finished builds are skipped without error, and a',
      'build still holding an open escalation is left to ab answer. Output names every',
      'build that received a request and the intake value written; --json emits that',
      'summary as JSON. Re-running it changes nothing further. A phase session may not',
      'use --all. A failure exits nonzero and reports which builds were requested and',
      'which were not.',
    ]),
  },
  {
